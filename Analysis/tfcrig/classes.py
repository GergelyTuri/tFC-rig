...
class Trial:
    ...
    def __post_init__(self):
        ...
        required_columns = {"mouse_id", "absolute_time", "event", 
                        "trial_number", "trial_time", "trial_type"}
    ...
    def get_absolute_licks_per_trial(self) -> pd.DataFrame:
        ...
        """
        return self.trial_df[self.trial_df["event"] == "Lick"].groupby(
            "trial_number"
        ).size()
    ...
    def get_lick_rate_per_trial(self) -> pd.DataFrame:
        ...
        """
        lick_counts = self.get_absolute_licks_per_trial()
        trial_duration = self.trial_df.groupby("trial_number")[
            "trial_time"].max()
    ...
    def get_avg_interlick_interval(self) -> pd.Series:
        ...
        lick_events['interlick_interval'] = lick_events.groupby(
            "trial_number")["trial_time"].diff()
        #calculate average interlick interval 
        return lick_events.groupby(
            "trial_number")["interlick_interval"].mean().fillna(0)
    ...
    def compute_lick_metrics(self, session_metadata: Dict[str, Any]) -> pd.DataFrame:
        ...
        trace_duration = (
            session_metadata.get("AIR_PUFF_START_TIME", 0) 
        ...
        for trial_number, trial_data in self.trial_df.groupby("trial_number"):
            ...
            for _, row in trial_data.iterrows():
                ...
                if row["event"] == "Water off" and current_reward is not None:
                    rewards.append((current_reward, row["trial_time"]))
                    current_reward = None

            pre_tone_rewards = sum(1 for start, end in rewards if start < pre_tone_end)
            tone_rewards = sum(1 for start, end in rewards if pre_tone_end <= start < tone_end)
            trace_rewards = sum(1 for start, end in rewards if tone_end <= start < trace_end)
    ...
    def compute_lick_delays(self, session_metadata: Dict[str, Any]) -> pd.DataFrame:
        ...
        for trial_number, trial_data in self.trial_df.groupby("trial_number"):
            ...
            post_trace_delay_from_trial_start = (
                post_trace_first_lick 
                if pd.notna(post_trace_first_lick) else None
            )
            ...
            tone_delay_from_period_start = (
                (tone_first_lick - tone_start) 
                if pd.notna(tone_first_lick) else None
            )

            trace_delay_from_period_start = (
                (trace_first_lick - trace_start) 
                if pd.notna(trace_first_lick) else None
            )
