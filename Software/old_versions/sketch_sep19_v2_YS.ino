...
void loop() {
  ...
  if (sessStart == 1) {
    ...
    for (int i = 0; i < 4; i++){   //for loop is probably the better way to do this.
...
void Cue() {  
  ...
  if (randomValue == 1) {  //CS-
    do {
      ...
      //Pulsed tones CS-
      if (toneNon == 1 && 
          currentTime - previousTime >= (toneState == LOW ? paulsedToneOffDuration : paulsedToneOnDuration)) {  //if tone off, compare with paulsed off duration, else, pulsed on duration
